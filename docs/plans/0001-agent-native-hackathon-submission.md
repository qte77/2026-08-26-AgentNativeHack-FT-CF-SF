...
| 9 | Register team / confirm team status | owner | **REOPENED — corrected after owner clarified `/events/hackathon/apply` was the (now-closed) event-application form, not solution submission; the earlier "confirmation email" was for that application, not a project submission. Team/registration status is unconfirmed again. The `ic_hack_*` CLI/MCP auth path is blocked from autonomous agent execution by Claude Code's own classifier — needs the owner's own hands (`! npx -y @immersivecommons/cli auth ...`, approve the device code, then either the owner or the agent with the resulting token can call read-only status tools).** |
| 10 | Submit the solution — **`ic_hack_submit` MCP tool is the ONLY submission path, no web-form alternative exists** (confirmed against a fresh fetch of the live page: `/apply` was event-application only) | owner then agent | Owner completes the CLI auth device-code approval; the resulting token calls `ic_hack_submit` with `title`, `blurb`, `repo_url`, `demo_url`, `agent_surface` (see "Live deployment" line above) and a confirmed non-error response is captured, not just a human's belief that it worked |
| 10a | Upgrade `agent_surface` from generic `"HTTP API"` to a real, working **A2A endpoint** before submitting | agent | **The page states directly: "agent_surface is what the 30-point band actually scores"** — the single highest-weighted rubric line in both tracks. Currently declared as a non-matching generic value. Build a minimal real A2A surface (agent card + message endpoint) on the Worker, verify it works, then use `"A2A endpoint"` as the submitted value. Sequencing: submit once with current value first as a safety net (idempotent, overwrite-until-lock), then resubmit after this upgrades. |
